...
[용어 해석 — 매우 중요]
- 주어진 용어의 의미를 임의로 바꾸지 마라. 괄호 안 설명이 있으면 반드시 그 의미로 작성한다.
  (예: "DR (도메인 레이팅)" → SEO의 Domain Rating으로. Direct Response로 해석 금지.)
- 약어가 모호하면 괄호 설명을 최우선으로 따른다. 설명이 없을 때만 마케팅 맥락의 일반 의미로.
