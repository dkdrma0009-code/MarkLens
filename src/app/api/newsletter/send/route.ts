...
export async function POST(req: Request) {
  ...
  for (const { email } of subscribers) {
    try {
      ...
      await sendViaBrevo(email, subject, buildHtml(issue, unsubscribeUrl))
      sent++
    } catch (e) {
...
function buildHtml(issue: NewsletterIssue, unsubscribeUrl = ""): string {
    ...
    <tr><td style="padding:0 24px 20px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;border-radius:14px;overflow:hidden;">
        ...
        <tr><td style="padding:22px 26px 8px;">
          <p style="margin:0 0 6px;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#4f46e5;${F}">📡 01 · This Week's Signals</p>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f8f8;border-radius:14px;overflow:hidden;border:1px solid #eeeeee;">
        <tr>
          ...
          <td style="padding:22px 22px 18px;">
            <p style="margin:0 0 6px;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#0891b2;${F}">🔍 02 · Case of the Week</p>
    ...
    <tr><td style="padding:0 24px 20px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border-radius:14px;overflow:hidden;border:1px solid #bbf7d0;">
        <tr><td style="padding:20px 22px 16px;">
          <p style="margin:0 0 6px;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#059669;${F}">🤖 03 · AI Marketing Brief</p>
  ...
  function portfolioSection(content: string): string {
    ...
    <tr><td style="padding:0 24px 20px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf5ff;border-radius:14px;overflow:hidden;border:1px solid #e9d5ff;">
        <tr><td style="padding:20px 22px 16px;">
          <p style="margin:0 0 4px;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#7c3aed;${F}">📁 04 · Portfolio Insight</p>
  ...
  function careerSection(content: string): string {
    ...
    <tr><td style="padding:0 24px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#fffbeb;border-radius:14px;overflow:hidden;border:1px solid #fde68a;">
        <tr><td style="padding:20px 22px 16px;">
          <p style="margin:0 0 4px;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#d97706;${F}">🎯 05 · Career Lens</p>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <tr><td style="background:#0a0a0a;border-radius:16px 16px 0 0;padding:0;">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <td style="height:4px;background:#4f46e5;border-radius:16px 0 0 0;font-size:0;">&nbsp;</td>
        <td style="height:4px;background:#7c3aed;font-size:0;">&nbsp;</td>
        <td style="height:4px;background:#ec4899;border-radius:0 16px 0 0;font-size:0;">&nbsp;</td>
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr><td style="padding:30px 32px 26px;text-align:center;">
        <p style="margin:0 0 4px;font-size:10px;font-weight:600;color:#555;letter-spacing:0.2em;text-transform:uppercase;${F}">MarkLens Weekly${issueNum ? ` &nbsp;·&nbsp; Issue #${issueNum}` : ""}</p>
        <h1 style="margin:8px 0 12px;font-size:24px;font-weight:900;color:#fff;line-height:1.3;letter-spacing:-0.4px;${F}">${cleanTitle}</h1>
