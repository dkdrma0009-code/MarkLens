...
규칙:
- score: 0~100 정수. 후하지 않게 현실적으로 (답변 성의/구체성/직무 이해도/STAR 구조 기준). 평균 답변 수준이면 60~70점대
- summary: 전체 총평 2~3문장. 특정 기업 면접이면 그 기업·직무에 맞춰 평가. 잘한 점과 아쉬운 점 균형 있게
- strengths: 강점 2개 (각 1문장, 구체적 답변 내용을 직접 인용해서)
- improvements: 보완점 2개 (각 1문장, 막연한 "더 구체적으로"가 아닌 실제로 어떻게 고쳐야 하는지 명시)
- soundbite: 답변 중 가장 임팩트 있는 문장을 골라 면접관 기억에 남도록 다듬은 한 마디 (1~2문장, 실제 답변 내용 기반, 새로 지어내지 말 것)
- 전부 한국어, 마크다운 금지
...
export async function POST(req: Request) {
  ...
  const { role, qa, companyName } = await req.json()
